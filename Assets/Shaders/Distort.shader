...
Properties { 
_Refraction ("Refraction", Range (0.00, 1000.0)) = 1.0 _DistortTex ("Base (RGB)", 2D) = "white" {} } 
SubShader {
 Tags { 
 "RenderType"="Transparent" "Queue"="Overlay" 
  ...
  LOD 100
   ...
   CGPROGRAM
    ...
    #pragma vertex vert 
    fixed4 LightingNoLighting(SurfaceOutput s, fixed3 lightDir, fixed atten) { fixed4 c; c.rgb = s.Albedo; c.a = s.Alpha; return c; } 
      ...
      void vert (inout appdata_full v, out Input o) {
       ...
       float2 offset = distort * _Refraction * _GrabTexture_TexelSize.xy;
        ...
        float4 refrColor = tex2Dproj(_GrabTexture, IN.screenPos);
         o.Alpha = refrColor.a; o.Emission = refrColor.rgb; 
         }
          ENDCG 
          }
           FallBack "Diffuse" 
